...
def _ci(popt, pcov, cross_fn, n_samples=CI_SAMPLES):
    ...
    try:
        ...
        crossings = crossings[np.isfinite(crossings) & (crossings > 0) & (crossings < 1e6)]
...
def run_decay_model(parquet_path=None, output_dir=None):
    ...
    # Interpolate sparse NaNs before fitting
    recent["sint2000_norm"] = (
        recent["sint2000_norm"].interpolate(method="linear").ffill().bfill()
    )
    # Convert to "years from now" convention: τ = −age_bp
    tau_hist = -recent["age_bp"].values.astype(float)   # in [−5000, −500]
    vadm_hist = recent["sint2000_norm"].values.astype(float)
